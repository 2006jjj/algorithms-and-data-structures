...
Далее идут в строго возрастающем порядке координаты домов учеников — целые числа, не превосходящие 2 × 10^9 по модулю.
...
if (N < 0 || N > 100001)
  ...
  cout << "Пока здесь никто не живет";
else
{
  cout << "Введите первую точку координат:" << endl;
  cin >> i;
  if (i + N > (int)(abs(2 * pow(10, 9))))
    cout << "Error";
  else
  {
    for (int j = 0; j < N; j++)
