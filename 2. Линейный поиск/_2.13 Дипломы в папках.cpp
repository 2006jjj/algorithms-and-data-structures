/*Задача 2.13
В этом году Иван заканчивает школу и поступает в вуз. За время своей учебы он часто участвовал в олимпиадах по информатике и у него накопилось много дипломов. 
Иван раскладывал дипломы по папкам совершенно бессистемно, то есть любой диплом мог оказаться в любой из папок. К счастью, Иван помнит, сколько дипломов лежит в каждой из папок.
...
int findseconds(int[], int);
int main()
{setlocale(LC_ALL, ".1251"); system("color 0F"); srand(time(NULL));

const int MAX = 100;
int array[MAX], N=0;
cout << "Введите количество папок:"<<endl;
cin >> N;
cout << "Введите количество дипломов в каждой папке:" << endl;
for (int j = 0; j < N; j++)
	cin >> array[j];
...
cout << findseconds(array, N);

cout << endl; system("pause");return 0;
}

int findseconds(int array[], int N)
{
	int res = array[0], i = 0;
	for (int j = 1; j < N; j++)
	{
